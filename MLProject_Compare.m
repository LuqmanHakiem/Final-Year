...
load('trainedAgent.mat');  % if saved, or run training first
agentObj = agent;          % ensure Simulink can access this
...
mse_pid = mean((theta_pid).^2);  % assuming target theta = 0
...
theta_rl = simOut_RL.simout1.signals.values(:,1);  % Only theta
